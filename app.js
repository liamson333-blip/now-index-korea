...
function applyLanguage(language) {
  ...
  const strategyText = language === 'ko'
    ? {
        ...
        note: '필요한 과거 관측값이 쌓일 때까지 수익 금액을 표시하지 않습니다. 오늘의 점수를 과거 가격에 적용하면 룩어헤드 편향으로 결과가 왜곡되기 때문입니다.', kosdaq: 'KOSDAQ 투자 P&L', kospi: 'KOSPI 투자 P&L', strategy: 'NOW 전략 P&L',
      }
    : {
        ...
        note: 'No profit number is shown until the required historical observations exist. This avoids look-ahead bias: applying today’s score to old prices would make the result misleading.', kosdaq: 'P&L investing in KOSDAQ', kospi: 'P&L investing in KOSPI', strategy: 'P&L with NOW strategy',
      };
  const strategyTargets = { scoreStoryTitle: 'title', scoreStoryDescription: 'description', buyRule: 'buy', holdRule: 'hold', sellRule: 'sell', backtestTitle: 'backtest', backtestStatus: 'status', chartEmptyTitle: 'emptyTitle', chartEmptyText: 'emptyText', chartEndLabel: 'end', backtestNote: 'note', kosdaqLegend: 'kosdaq', kospiLegend: 'kospi', strategyLegend: 'strategy' };
...
function renderBacktest(data) {
  ...
  const hasThreeSeries = data.equity_curve.every((point) => point.kospi != null && point.kosdaq != null);
  if (!hasThreeSeries) return;
  ...
  const kosdaqValues = data.equity_curve.map((point) => Number(point.kosdaq ?? 100));
  const kospiValues = data.equity_curve.map((point) => Number(point.kospi ?? 100));
  const min = Math.min(...values, ...kosdaqValues, ...kospiValues, 100);
  const max = Math.max(...values, ...kosdaqValues, ...kospiValues, 100);
  ...
  const kosdaqPoints = makePoints(kosdaqValues);
  const kospiPoints = makePoints(kospiValues);
  area.querySelector('.chart-empty')?.remove();
  area.insertAdjacentHTML('afterbegin', `<svg class="backtest-line" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><polyline class="strategy-line" points="${points}" /><polyline class="kosdaq-line" points="${kosdaqPoints}" /><polyline class="kospi-line" points="${kospiPoints}" /></svg>`);
  if (status) {
    const formatReturn = (value) => value == null ? 'N/A' : `${value >= 0 ? '+' : ''}${Number(value).toFixed(2)}%`;
    status.textContent = `NOW ${formatReturn(data.return_pct)} · KOSDAQ ${formatReturn(data.kosdaq_return_pct)} · KOSPI ${formatReturn(data.kospi_return_pct)}`;
